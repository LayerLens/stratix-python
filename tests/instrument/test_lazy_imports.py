...
def test_adapter_packages_importable_without_framework() -> None:
    """The ``providers`` package must be importable without optional deps.

    It exposes only ``__init__.py`` documentation; concrete adapter
    modules are loaded by :class:`AdapterRegistry` on demand. The sister
    ``frameworks`` and ``protocols`` packages are landed by their own
    M-series PRs and intentionally omitted here.
    """
