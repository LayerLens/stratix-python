...
class TestAnthropicCreateWrap:
    ...
    def test_cache_metadata_captured(self) -> None:
        ...
        adapter.connect()
        client = _make_client(
            returns=_make_response(cache_creation=100, cache_read=200)
        )
        adapter.connect_client(client)
        client.messages.create(
            model="claude-sonnet-4-5-20250929",
...
class TestCostCalculation:
    def test_known_model_priced(self) -> None:
        ...
        adapter.connect()
        client = _make_client(
            returns=_make_response(input_tokens=1000, output_tokens=500)
        )
        adapter.connect_client(client)
        client.messages.create(
            model="claude-sonnet-4-5-20250929",
...
class TestAnthropicStreaming:
    def test_stream_emits_one_consolidated_invoke(self) -> None:
        ...
        wrapped = adapter._wrap_stream_response(
            events, model="claude-sonnet-4-5-20250929", params={}, start_ns=0
        )
        for _ in wrapped:
            pass
