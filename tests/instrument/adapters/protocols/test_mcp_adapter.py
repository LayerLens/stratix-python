...
def test_lifecycle_transitions() -> None:
    adapter = MCPExtensionsAdapter(org_id="test-org")
...
def test_on_structured_output_emits_event() -> None:
    ...
    payload = next(e for e in stratix.events if e["event_type"] == "protocol.tool.structured_output")["payload"]
...
def test_elicitation_request_response_lifecycle() -> None:
    ...
    response = next(e for e in stratix.events if e["event_type"] == "protocol.elicitation.response")["payload"]
