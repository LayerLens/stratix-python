"""Test SmolAgents adapter event emission.

Ported as-is from ``ateam/tests/adapters/smolagents/test_events.py``.
...
class TestSmolAgentsAdapterEvents:
    def test_on_on_run_start_emits_agent_input(self, adapter, mock_stratix):
        ...
        assert len(events) == 1
        assert events[0]["payload"]["framework"] == "smolagents"
    ...
    def test_capture_config_minimal_gates_l3_l5(self, mock_stratix):
        ...
        adapter = SmolAgentsAdapter(stratix=mock_stratix, capture_config=CaptureConfig.minimal())
        adapter.connect()
    ...
    def test_cross_cutting_always_emitted(self, mock_stratix):
        ...
        adapter.emit_dict_event(
            "agent.state.change", {"framework": "smolagents", "event_subtype": "test"}
        )
