"""Test OpenAI Agents adapter event emission.

Ported from ``ateam/tests/adapters/openai_agents/test_events.py``.
...
class TestOpenAIAgentsAdapterEvents:
    def test_on_on_run_start_emits_agent_input(self, adapter: OpenAIAgentsAdapter, mock_stratix: MockStratix) -> None:
        adapter.on_run_start(agent_name="test_agent", input_data="hello")
        events = mock_stratix.get_events("agent.input")
        assert len(events) == 1
        assert events[0]["payload"]["framework"] == "openai_agents"

    def test_on_on_run_end_emits_agent_output(self, adapter: OpenAIAgentsAdapter, mock_stratix: MockStratix) -> None:
        adapter.on_run_start(agent_name="test_agent", input_data="hello")
        adapter.on_run_end(agent_name="test_agent", output="response")
        events = mock_stratix.get_events("agent.output")
        assert len(events) == 1

    def test_on_tool_use_emits_tool_call(self, adapter: OpenAIAgentsAdapter, mock_stratix: MockStratix) -> None:
        adapter.on_tool_use(
            tool_name="test_tool",
    ...
    def test_on_llm_call_emits_model_invoke(self, adapter: OpenAIAgentsAdapter, mock_stratix: MockStratix) -> None:
        adapter.on_llm_call(
            provider="openai",
            model="gpt-4o",
            tokens_prompt=100,
    ...
    def test_capture_config_minimal_gates_l3_l5(self, mock_stratix: MockStratix) -> None:
        adapter = OpenAIAgentsAdapter(stratix=mock_stratix, capture_config=CaptureConfig.minimal())
        adapter.connect()
    ...
    def test_cross_cutting_always_emitted(self, mock_stratix: MockStratix) -> None:
        ...
        adapter.emit_dict_event(
            "agent.state.change",
            {"framework": "openai_agents", "event_subtype": "test"},
        )
        assert len(mock_stratix.get_events("agent.state.change")) == 1

    def test_error_in_output(self, adapter: OpenAIAgentsAdapter, mock_stratix: MockStratix) -> None:
...
# Reference TEST_ORG_ID to keep the import live for downstream maintainers
# (the conftest fixture passes it implicitly; the constant is exported
# here so test files that build adapters directly can reuse it).
_TEST_ORG_ID_REF = TEST_ORG_ID
