...
Ported from ``ateam/tests/adapters/bedrock_agents/test_events.py``.
"""
...
class TestBedrockAgentsAdapterEvents:
    def test_on_on_invoke_start_emits_agent_input(
    ...
    def test_on_on_invoke_end_emits_agent_output(
        self, adapter: BedrockAgentsAdapter, mock_stratix: Any
    ) -> None:
        adapter.on_invoke_start(agent_id="agent-123", input_text="hello")
    ...
    def test_capture_config_minimal_gates_l3_l5(self, mock_stratix: Any) -> None:
        adapter = BedrockAgentsAdapter(
            stratix=mock_stratix, capture_config=CaptureConfig.minimal()
        )
        adapter.connect()
    ...
    def test_cross_cutting_always_emitted(self, mock_stratix: Any) -> None:
        adapter = BedrockAgentsAdapter(
            stratix=mock_stratix, capture_config=CaptureConfig.minimal()
        )
        adapter.connect()
        adapter.emit_dict_event(
            "agent.state.change", {"framework": "bedrock_agents", "event_subtype": "test"}
        )
