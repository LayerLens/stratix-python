"""Tests for LayerLens LangChain Agent Instrumentation.

Ported as-is from ``ateam/tests/adapters/langchain/test_agents.py``.
...
* ``MockStratix.org_id == "test-org"`` retained where present
* ``@pytest.mark.asyncio`` tests rewritten to use ``asyncio.run`` because
  ``pytest-asyncio`` is not a stratix-python test dependency (mirrors the
  parallel langgraph-test-restoration approach).
"""
...
from layerlens.instrument.adapters.frameworks.langchain.agents import (
    AgentStep,
    AgentTracer,
    TracedAgent,
    AgentExecution,
...
class TestTracedAgent:
    ...
    def test_invoke_executes_agent(self) -> None:
    ...
    def test_invoke_emits_input_event(self) -> None:
    ...
    def test_invoke_emits_output_event(self) -> None:
    ...
    def test_attribute_proxying(self) -> None:
        ...
        agent = MockAgent()
        agent.custom_attr = "value"  # type: ignore[attr-defined]
        traced = TracedAgent(agent)
...
class TestTracedAgentAsync:
    """Async tests for TracedAgent (driven through ``asyncio.run``)."""

    def test_ainvoke_executes_agent(self) -> None:
...
class TestInstrumentAgent:
    ...
    def test_creates_traced_agent(self) -> None:
...
class TestAgentTracer:
    ...
    def test_trace_creates_traced_agent(self) -> None:
    ...
    def test_get_all_executions(self) -> None:
...
class TestAgentStep:
    ...
    def test_step_optional_fields(self) -> None:
