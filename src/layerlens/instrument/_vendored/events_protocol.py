"""Vendored snapshot of ``stratix.core.events.protocol``.

Source: ``A:/github/layerlens/ateam/stratix/core/events/protocol.py``
...
class TaskSubmittedEvent(BaseModel):
    ...
    task_type: Optional[str] = Field(
        default=None,
        description="Semantic task type (from skill definition)",
    )
    submitter_agent_id: Optional[str] = Field(
        default=None,
        description="Agent submitting the task",
    )
    ...
    protocol_origin: str = Field(
        default="a2a",
        description="Protocol origin: a2a | acp",
    )
    message_role: str = Field(
        default="user",
        description="Message role: user | agent",
    )
...
class TaskCompletedEvent(BaseModel):
    ...
    artifact_hashes: list[str] = Field(
        ...
        description="sha256:<hex> per artifact",
    )
    error_code: Optional[str] = Field(default=None, description="A2A error code if failed")
    error_message: Optional[str] = Field(default=None, description="Error message if failed")
    duration_ms: Optional[float] = Field(
        default=None,
        description="Wall time from submitted to completed",
    )
...
class AsyncTaskEvent(BaseModel):
    ...
    originating_tool_call_span_id: Optional[str] = Field(
        default=None,
        description="Links to the originating tool.call span",
    )
    status: str = Field(
    ...
    progress_pct: Optional[float] = Field(
        default=None,
        description="0.0-100.0 progress if reported",
    )
    timeout_ms: Optional[int] = Field(default=None, description="Configured timeout")
    elapsed_ms: Optional[float] = Field(default=None, description="Time since creation")
    ...
    def create(
        ...
        protocol: str,
        *,
        originating_tool_call_span_id: Optional[str] = None,
...
class ProtocolStreamEvent(BaseModel):
    ...
    agui_event_type: Optional[str] = Field(
        default=None,
        description="AG-UI event type (e.g. TEXT_MESSAGE_CONTENT)",
    )
    sequence_in_stream: int = Field(
        description="Position within the SSE stream",
    )
    payload_summary: Optional[str] = Field(
        default=None,
        description="Truncated payload for low-verbosity capture",
    )
    ...
    def create(
        ...
        payload_hash: str,
        *,
        agui_event_type: Optional[str] = None,
...
class ElicitationRequestEvent(BaseModel):
    ...
    schema_ref: Optional[str] = Field(
        default=None,
        description="JSON Schema $id for the requested input",
    )
...
class StructuredToolOutputEvent(BaseModel):
    ...
    schema_id: Optional[str] = Field(
        default=None,
        description="JSON Schema $id reference",
    )
...
class McpAppInvocationEvent(BaseModel):
    ...
    result_hash: Optional[str] = Field(
        default=None,
        description="sha256 of user interaction result",
    )
