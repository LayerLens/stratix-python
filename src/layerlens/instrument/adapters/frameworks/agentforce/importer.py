...
and normalizes it to LayerLens canonical events.
...
# Regex for Salesforce record IDs (exactly 15 or 18 char alphanumeric)
...
class AgentForceImporter:
    ...
    def import_sessions(
        ...
        agent_type: str | None = None,  # noqa: ARG002 — reserved for SOQL filter wiring
        channel_type: str | None = None,  # noqa: ARG002 — reserved for SOQL filter wiring
        limit: int | None = None,
        ...
        Returns:
            Tuple of (list of LayerLens events, ImportResult summary)
        """
        result = ImportResult()
        ...
        logger.info(
            "AgentForce import complete: %d sessions, %d events generated",
    ...
    def _query_related(
        ...
        for i in range(0, len(parent_ids), self._batch_size):
            ...
            # Validate IDs to prevent SOQL injection
            safe_ids = [self._validate_sf_id(pid) for pid in batch]
    ...
    @staticmethod
    def _validate_sf_id(value: str) -> str:
        """Validate a Salesforce ID format (15 or 18 char alphanumeric).

        Raises:
            ValueError: If the value does not match the Salesforce ID format.
        """
