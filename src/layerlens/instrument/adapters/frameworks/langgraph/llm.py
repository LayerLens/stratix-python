...
class TracedLLM:
    ...
    Usage:
        from langchain_openai import ChatOpenAI  # type: ignore[import-untyped,unused-ignore]
    ...
    def _extract_token_usage(self, response: Any) -> dict[str, int] | None:
        ...
        if hasattr(response, "response_metadata"):
            ...
            if isinstance(metadata, dict) and "usage" in metadata:
                return metadata["usage"]  # type: ignore[no-any-return]

        # Try usage_metadata
        if hasattr(response, "usage_metadata"):
            return response.usage_metadata  # type: ignore[no-any-return]
    ...
    def _emit_model_invoke(self, invocation: LLMInvocation) -> None:
        ...
        if self._adapter is not None:
            try:
                from layerlens.instrument._vendored.events import ModelInvokeEvent

                typed_payload = ModelInvokeEvent.create(  # type: ignore[call-arg,unused-ignore]
