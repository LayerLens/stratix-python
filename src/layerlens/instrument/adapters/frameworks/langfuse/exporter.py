...
Reverse-maps LayerLens events to Langfuse traces and pushes them via the API.
"""
...
class TraceExporter:
    """
    Export pipeline for LayerLens -> Langfuse.

    Steps:
    1. Group LayerLens events by trace ID
    2. Reverse-map to Langfuse trace + observations
    3. Create trace and observations via Langfuse API
    4. Tag with ``layerlens-exported`` (and the legacy ``stratix-exported``
       alias) to prevent re-import on the next import cycle.
    """
    ...
    def export_traces(
        ...
        for trace_id in ids_to_export:
            ...
            # Map LayerLens events to Langfuse structure
            try:
