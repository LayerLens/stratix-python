"""
Langfuse <-> LayerLens Bidirectional Field Mapper

Maps Langfuse trace/observation structures to LayerLens canonical events
and vice versa.
"""
...
class LangfuseToLayerLensMapper:
    """
    Maps Langfuse traces and observations to LayerLens canonical event dicts.

    Each Langfuse trace produces multiple LayerLens events:
    ...
    def map_trace(self, trace: dict[str, Any]) -> list[dict[str, Any]]:
        """
        Map a complete Langfuse trace (with observations) to LayerLens events.

        Args:
            trace: Langfuse trace dict from the API, including nested observations.

        Returns:
            List of LayerLens event dicts ready for ingestion.
        """
        trace_id = trace.get("id", str(uuid.uuid4()))
        events: list[dict[str, Any]] = []
        timestamp = trace.get("timestamp", datetime.now(UTC).isoformat())
        seq = 0
    ...
    def _map_observation(
        ...
        """Map a single Langfuse observation to LayerLens event(s)."""
    ...
    def _make_event(
        ...
        """Construct a normalized LayerLens event dict."""
        event: dict[str, Any] = {
...
class LayerLensToLangfuseMapper:
    """
    Maps LayerLens canonical events back to Langfuse trace/observation structures.

    Used for exporting LayerLens traces to Langfuse.
    """
    ...
    def map_events_to_trace(
        ...
        """
        Map a list of LayerLens events to a Langfuse trace with observations.

        Returns a dict with 'trace' (trace body) and 'observations' (list of observations).
        """
        trace_id = trace_id or str(uuid.uuid4())

        # Tag with both the canonical ``layerlens-exported`` name and the
        # legacy ``stratix-exported`` alias so traces remain discoverable
        # by clients written against the pre-rename adapter. The importer
        # likewise recognises both tags for loop prevention.
        trace_body: dict[str, Any] = {
            "id": trace_id,
            "name": "layerlens-export",
            "tags": ["layerlens-exported", "stratix-exported"],
            "metadata": {
                "layerlens_trace_id": trace_id,
                "stratix_trace_id": trace_id,  # deprecated alias
            },
        }
    ...
    def _attach_cost(
        ...
        for obs in reversed(observations):
            if obs.get("type") == "GENERATION":  # noqa: SIM102
