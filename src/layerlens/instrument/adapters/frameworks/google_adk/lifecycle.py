...
class GoogleADKAdapter(BaseAdapter):
    ...
    async def execute_replay_via_factory(
        ...
        """Re-execute ``trace`` through a fresh Google ADK agent.

        :meth:`instrument_agent` attaches the adapter's
        ``before_/after_*`` callbacks so re-execution emits the same
        event shape as the original capture.
        """
    ...
    def _invoke_for_replay(
        ...
        """Google ADK invocation: ``run`` / ``run_async`` (or duck-typed)."""
        if hasattr(agent, "run_async") and callable(agent.run_async):
            return agent.run_async(inputs)
