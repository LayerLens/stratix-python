...
class EventSink(ABC):
    ...
    :meth:`BaseAdapter._post_emit_success` and persist or forward them.
    """
    ...
    def send(self, event_type: str, payload: Dict[str, Any], timestamp_ns: int) -> None:
        ...
        Args:
            ...
            payload: Serialized event payload dict.
...
class IngestionPipelineSink(EventSink):
    ...
    * **buffered**: events are collected and ingested on
    ...
    def send(self, event_type: str, payload: Dict[str, Any], timestamp_ns: int) -> None:
        ...
        formatted = self._format_event(event_type, payload, timestamp_ns)

        if self._buffered:
            self._buffer.append(formatted)
        else:
            try:
                self._pipeline.ingest([formatted], tenant_id=self._tenant_id)
            except Exception:
                logger.debug(
