"""
Ollama LLM Provider Adapter (ADP-058)

Wraps the Ollama Python SDK to intercept chat, generate, and embeddings calls.
All API costs are $0.00 (local). Optional infra cost tracking via compute duration.
"""
...
class OllamaAdapter(LLMProviderAdapter):
    """
    STRATIX adapter for the Ollama Python SDK.

    Wraps ollama.chat(), ollama.generate(), and ollama.embeddings() calls.
    API cost is always $0.00 (local inference). Optionally tracks infra cost
    from compute duration if cost_per_second is configured.
    """
    ...
    def connect(self) -> None:
        ...
        self._status = self._status.HEALTHY
    ...
    def _wrap_call(self, original: Any, method_name: str) -> Any:
        ...
        def traced_call(*args: Any, **kwargs: Any) -> Any:
            ...
            if method_name == "chat":
                ...
                if prompt:
                    input_messages = [{"role": "user", "content": str(prompt)[:10_000]}]
            ...
            try:
                ...
                try:
                    adapter._emit_model_invoke(
                        ...
                        error=str(exc),
                        metadata={"method": method_name, "endpoint": adapter._endpoint},
            ...
            try:
                ...
                ollama_metadata: dict[str, Any] = {"method": method_name, "endpoint": adapter._endpoint}
        ...
        traced_call._stratix_original = original
        return traced_call
    ...
    @staticmethod
    def _extract_output_message(response: Any, method_name: str) -> dict[str, str] | None:
    ...
    def _calculate_infra_cost(self, response: Any) -> float | None:
        ...
        total_ns = 0
        if isinstance(response, dict):
            total_ns = (response.get("eval_duration", 0) or 0) + (response.get("prompt_eval_duration", 0) or 0)
        else:
            total_ns = (getattr(response, "eval_duration", 0) or 0) + (getattr(response, "prompt_eval_duration", 0) or 0)
