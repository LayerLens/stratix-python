...
class STRATIXCallbackHandler(BaseAdapter):
    """
    ...
    Usage (new):
        ...
        from layerlens.instrument.adapters.langchain import STRATIXCallbackHandler
    ...
    Usage (legacy — still supported):
        handler = STRATIXCallbackHandler(stratix_instance=stratix)
        llm = ChatOpenAI(callbacks=[handler])
    """
    ...
    def __init__(
        ...
        if capture_config is None:
            any_legacy = (
                not emit_llm_events
                or not emit_tool_events
                or not emit_agent_events
            )
    ...
    def get_adapter_info(self) -> AdapterInfo:
        return AdapterInfo(
            ...
            description="STRATIX adapter for LangChain framework (callback-based)",
        )

    def serialize_for_replay(self) -> ReplayableTrace:
        trace_id = str(uuid.uuid4())
        return ReplayableTrace(
            adapter_name="STRATIXCallbackHandler",
            framework=self.FRAMEWORK,
            trace_id=trace_id,
    ...
    async def execute_replay(
        ...
        replay_handler = STRATIXCallbackHandler(event_sinks=[])
