...
class MemoryMutationTracker:
    ...
    def __init__(self, stratix_instance: Any = None):
        """
...
class _MemoryTrackingContext:
    ...
    def __enter__(self) -> "_MemoryTrackingContext":
        self._before_snapshot = self._adapter.snapshot()
        return self

    def __exit__(self, exc_type: Any, exc_val: Any, exc_tb: Any) -> None:
        after_snapshot = self._adapter.snapshot()

        if self._adapter.has_changed(self._before_snapshot, after_snapshot):
            diff = self._adapter.diff(self._before_snapshot, after_snapshot)

            mutation = {
                "operation": self._operation,
                "before_hash": self._before_snapshot.hash,
            ...
            if self._stratix and hasattr(self._stratix, "emit"):
                self._stratix.emit("agent.state.change", {
                    "memory_type": self._before_snapshot.memory_type,
                    "before_hash": self._before_snapshot.hash,
                    "after_hash": after_snapshot.hash,
                    "operation": self._operation,
                })
