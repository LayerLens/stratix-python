...
class AnthropicAdapter(LLMProviderAdapter):
    ...
    def _wrap_messages_create(self, original: Any) -> Any:
        ...
        def traced_create(*args: Any, **kwargs: Any) -> Any:
            ...
            if is_stream:
                return adapter._wrap_stream_response(
                    response, model, params, start_ns, input_messages
                )
    ...
    def _wrap_messages_stream(self, original: Any) -> Any:
        ...
        def traced_stream(*args: Any, **kwargs: Any) -> Any:
            ...
            return _TracedStreamManager(
                adapter, stream_ctx, model, params, start_ns, input_messages
            )
                    ...
                    try:
                        ...
                        if accumulated_tool_calls:
                            adapter._emit_tool_calls(
                                accumulated_tool_calls, parent_model=model
                            )
                    except Exception:
                        logger.warning(
                            "Error emitting Anthropic stream events", exc_info=True
                        )
                    raise
