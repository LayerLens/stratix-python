...
class ProtocolCertificationSuite:
        ...
        if not issubclass(cls, BaseProtocolAdapter):
            results.append(
                CheckResult(
                    name="instantiation",
                    passed=False,
                    message=f"{cls.__name__} is not a BaseProtocolAdapter subclass; skipping"
                    f" lifecycle"
                    f" checks",
                )
            )
    ...
    def _check_adapter_info_returns_type(self, cls: type) -> CheckResult:
        ...
        from layerlens.instrument.adapters._base.adapter import AdapterInfo
    ...
    def _check_serialize_for_replay_returns_type(self, cls: type) -> CheckResult:
        ...
        from layerlens.instrument.adapters._base.adapter import ReplayableTrace
        ...
        try:
            ...
            return CheckResult(
                ...
                passed=ok,
                message=(
                    f"serialize_for_replay() returns"
                    f" ReplayableTrace(adapter_name={trace.adapter_name!r})"
                    if ok
