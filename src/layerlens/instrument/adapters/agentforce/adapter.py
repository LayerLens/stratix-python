...
class AgentForceAdapter(BaseAdapter):
    ...
    Provides the standard STRATIX adapter lifecycle (connect/disconnect/health_check)
    around the AgentForce importer, routing imported events through the BaseAdapter
    circuit breaker and CaptureConfig pipeline.
        ...
        if self._connection is None:
            if self._credentials is None:
                raise SalesforceAuthError(
                    "Either 'credentials' or 'connection' must be provided"
                )
    ...
    def health_check(self) -> AdapterHealth:
        ...
        message = None
        if self._connection and self._credentials:
    ...
    def get_adapter_info(self) -> AdapterInfo:
        return AdapterInfo(
            ...
            description="STRATIX adapter for Salesforce AgentForce trace import",
        )
