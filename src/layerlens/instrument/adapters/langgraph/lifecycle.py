...
if TYPE_CHECKING:
    from layerlens.instrument.adapters.langgraph.handoff import HandoffDetector
...
class STRATIXLangGraphAdapter(BaseAdapter):
    """
    ...
    Usage (legacy — still supported):
        adapter = STRATIXLangGraphAdapter(stratix_instance=stratix)
        traced_graph = adapter.wrap_graph(my_graph)
    """
    ...
    def get_adapter_info(self) -> AdapterInfo:
        return AdapterInfo(
            ...
            description="STRATIX adapter for LangGraph agent framework",
        )

    def serialize_for_replay(self) -> ReplayableTrace:
        trace_id = str(uuid.uuid4())
        return ReplayableTrace(
            adapter_name="STRATIXLangGraphAdapter",
            framework=self.FRAMEWORK,
            trace_id=trace_id,
...
class _TracedGraph:
    ...
    def __init__(
        self,
        graph: Any,
        adapter: STRATIXLangGraphAdapter,
    ...
    def _get_graph_id(self) -> str:
        ...
        if hasattr(self._graph, "name"):
            ...
            return self._graph.__class__.__name__
        return "langgraph"
