...
class ToolTracer:
    ...
    def _capture_input(self, args: tuple, kwargs: dict) -> dict[str, Any]:
    ...
    def _emit_tool_call(self, execution: ToolExecution) -> None:
        ...
        if self._adapter is not None:
            try:
                from layerlens.instrument.schema.events import ToolCallEvent, IntegrationType
                typed_payload = ToolCallEvent.create(
                    tool_name=execution.tool_name,
...
class LangGraphToolNode:
    ...
    def __call__(self, state: dict[str, Any]) -> dict[str, Any]:
        ...
        if self._state_key:
            tool_input = state.get(self._state_key)
        else:
            tool_input = state
