...
class StateInfo(BaseModel):
    ...
    type: StateType = Field(
        description="Type of state (internal or ephemeral)"
    )
    before_hash: str = Field(
        description="SHA-256 hash of state before change"
    )
    after_hash: str = Field(
        description="SHA-256 hash of state after change"
    )
...
class AgentStateChangeEvent(BaseModel):
    ...
    event_type: str = Field(
        default="agent.state.change",
        description="Event type identifier"
    )
    state: StateInfo = Field(
        description="State change information"
    )
...
class CostInfo(BaseModel):
    ...
    api_cost_usd: float | str | None = Field(
        default=None,
        description="API cost in USD (or 'unavailable')"
    )
    infra_cost_usd: float | str | None = Field(
        default=None,
        description="Infrastructure cost in USD (or 'unavailable')"
    )
...
class ViolationInfo(BaseModel):
    ...
    failed_sequence_id: int | None = Field(
        default=None,
        description="Sequence ID where violation occurred"
    )
...
class PolicyViolationEvent(BaseModel):
    ...
    def create(
        ...
        failed_sequence_id: int | None = None,
        ...
        Args:
            ...
            failed_sequence_id: Sequence ID of failure
            details: Additional details

        Returns:
            PolicyViolationEvent instance
        """
        return cls(
...
class AgentHandoffEvent(BaseModel):
    ...
    handoff_context_hash: str = Field(
        description="SHA-256 hash of the handoff context"
    )
    context_privacy_level: str = Field(
        default="cleartext",
        description="Privacy level of the handoff context"
    )
    ...
    def create(
        ...
        Args:
            ...
            handoff_context_hash: Hash of the context
            context_privacy_level: Privacy level of context

        Returns:
            AgentHandoffEvent instance
        """
        return cls(
