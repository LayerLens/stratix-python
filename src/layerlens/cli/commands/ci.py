...
def ci() -> None:
    ...
    Examples:
      layerlens ci report
      layerlens ci report --format json
      layerlens ci report --output summary.md
    """
...
def ci_report(ctx: click.Context, output_file: str | None, limit: int, dry_run: bool) -> None:
    ...
    Examples:
      layerlens ci report
      layerlens ci report --output summary.md
      layerlens ci report --limit 20 --format json
      layerlens ci report >> $GITHUB_STEP_SUMMARY
    """
    if dry_run:
...
def _build_markdown_report(evaluations: list) -> str:
    ...
    lines.append("")
    lines.append("---")
    lines.append("*Generated by `layerlens ci report`*")
