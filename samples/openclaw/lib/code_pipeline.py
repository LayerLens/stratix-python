...
class StageResult:
    ...
    def __init__(
        self, stage: str, output: str,
        metadata: dict[str, Any] | None = None, duration_ms: int = 0,
    ) -> None:
        self.stage = stage
        self.output = output
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "stage": self.stage, "output": self.output[:500],
            "metadata": self.metadata, "duration_ms": self.duration_ms,
        }
...
class PipelineIteration:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "iteration": self.iteration, "verdict": self.verdict,
...
def _simulate_coder(task: str, iteration: int, feedback: str = "") -> StageResult:
    ...
    if iteration >= 2:
        lines.extend(['    # Input validation added based on reviewer feedback',
                       '    if not data:', '        raise ValueError("Input data cannot be empty")'])
    if iteration >= 3:
    ...
    if iteration >= 2:
        lines.extend([f'def test_{func_name}():', f'    """Tests for {func_name}."""',
                       f'    assert {func_name}({{"key": "value"}}) is not None',
                       f'    assert {func_name}({{"items": [1,2,3]}}) is not None'])
    if iteration >= 3:
        lines.extend([f'    # Edge case tests', f'    import pytest',
                       f'    with pytest.raises(ValueError):', f'        {func_name}(None)',
                       f'    with pytest.raises(ValueError):', f'        {func_name}({{}})'])
    code = "\n".join(lines)
    duration = int((time.monotonic() - start) * 1000) + 150 * iteration
    return StageResult(stage="coder", output=code,
        metadata={"function_name": func_name, "line_count": len(lines),
                   "iteration": iteration, "incorporated_feedback": bool(feedback)},
...
def _simulate_reviewer(code: str, task: str, iteration: int) -> StageResult:
    start = time.monotonic()
    if iteration == 1:
        comments = ["Missing input validation.", "No error handling for edge cases.",
                     "Consider adding type hints.", "Security: sanitize external inputs."]
    elif iteration == 2:
        comments = ["Input validation looks good.", "Consider more comprehensive test coverage.",
    ...
    return StageResult(stage="reviewer", output=review_text,
...
def _simulate_tester(code: str, task: str, iteration: int) -> StageResult:
    ...
    test_output = (f"Test Results (iteration {iteration}):\n  Total:  {total_tests}\n"
                   f"  Passed: {passed}\n  Failed: {failed}\n"
    ...
    return StageResult(stage="tester", output=test_output,
        metadata={"total_tests": total_tests, "passed": passed, "failed": failed,
    ...
    def execute(self, task: str) -> dict[str, Any]:
        ...
        for iteration in range(1, self.max_iterations + 1):
            ...
            judge_result = self.judge.evaluate(
                trace_id=trace_id, output=coder_result.output,
                context={"task": task, "iteration": iteration,
                         "review_comments": reviewer_result.output,
                         "test_results": tester_result.output})

            judge_stage = StageResult(stage="judge", output=judge_result["rationale"],
                metadata={"scores": judge_result["scores"],
            ...
            feedback = (f"Previous iteration scored {judge_result['aggregate_score']:.1f}. "
                        f"Suggestions: {'; '.join(suggestions)}")

        return {
            "task": task, "iterations": [it.to_dict() for it in self._iterations],
            "final_verdict": final_verdict, "final_score": final_score,
            "total_iterations": len(self._iterations),
            "gate_threshold": self.judge.gate_threshold,
            "passed": final_verdict == "PASS", "final_evaluation": final_result,
        }
